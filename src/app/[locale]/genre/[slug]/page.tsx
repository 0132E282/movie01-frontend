...
import { MOVIES, GENRES, slugify } from "@/data/movies";
...
function GenreContent({ slug }: { slug: string }) {
  ...
  const initialGenreName = useMemo(() => GENRES.find(g => slugify(g) === slug) || "Tất cả", [slug]);

  const [genre, setGenre] = useState(initialGenreName);
  ...
  useEffect(() => { setGenre(initialGenreName); setPage(1); }, [initialGenreName]);
  ...
  const getDynamicTitle = () => {
    ...
    return "Phim Theo Thể Loại";
  };

  const activeFiltersCount = [
    genre !== initialGenreName, 
    year !== "Tất cả", 
    quality !== "Tất cả", 
    country !== "Tất cả"
...
export default function GenreRoute({ params }: { params: Promise<{ slug: string; locale: string }> }) {
  ...
  return (
    <MainLayout hideBreadcrumb={false}>
      <Navbar />
      <div>
        <Suspense fallback={<div className="p-10 text-center text-text-muted py-20 font-bold">Đang tải phim theo thể loại...</div>}>
