...
import { MOVIES, COUNTRIES, slugify } from "@/data/movies";
...
function CountryContent({ slug }: { slug: string }) {
  ...
  const initialCountryName = useMemo(() => COUNTRIES.find(c => slugify(c) === slug) || "Tất cả", [slug]);
  ...
  useEffect(() => { setCountry(initialCountryName); setPage(1); }, [initialCountryName]);
    ...
    <div className="px-10 pt-6 pb-12 max-w-[1600px] mx-auto">
      <div className="flex items-center gap-4 mb-10 flex-wrap">
        ...
        <div className="flex items-center gap-2 text-text-muted text-[13.5px] font-medium bg-bg-2 border border-border px-4 py-2 rounded-full shadow-inner">
          ...
          {filtered.length} bộ phim được find found được tìm thấy
        </div>
      </div>
...
export default function CountryRoute({ params }: { params: Promise<{ slug: string; locale: string }> }) {
  ...
  return (
    <MainLayout hideBreadcrumb={false}>
      <Navbar />
      <div>
        <Suspense fallback={<div className="p-10 text-center text-text-muted py-20 font-bold">Đang tải phim theo quốc gia...</div>}>
