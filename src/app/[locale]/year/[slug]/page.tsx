"use client";
import { use, Suspense, useState, useMemo, useEffect } from "react";
import { MOVIES, YEARS, slugify } from "@/data/movies";
...
function YearContent({ slug }: { slug: string }) {
  const router = useRouter();
  const { favorites, toggleFavorite, addToHistory } = useAppContext();
  
  const initialYearName = useMemo(() => YEARS.find(y => slugify(y) === slug) || "Tất cả", [slug]);

  const [genre, setGenre] = useState("Tất cả");
  const [country, setCountry] = useState("Tất cả");
  const [year, setYear] = useState(initialYearName);
  const [quality, setQuality] = useState("Tất cả");
  ...
  useEffect(() => { setYear(initialYearName); setPage(1); }, [initialYearName]);
  ...
  const getDynamicTitle = () => {
    ...
    return "Phim Theo Năm Phát Hành";
  };

  const activeFiltersCount = [
    genre !== "Tất cả", 
    year !== initialYearName, 
    quality !== "Tất cả", 
    country !== "Tất cả"
...
export default function YearRoute({ params }: { params: Promise<{ slug: string; locale: string }> }) {
  ...
  return (
    <MainLayout hideBreadcrumb={false}>
      <Navbar />
      <div>
        <Suspense fallback={<div className="p-10 text-center text-text-muted py-20 font-bold">Đang tải phim theo năm...</div>}>
          <YearContent slug={slug} />
        </Suspense>
      </div>
    </MainLayout>
  );
