...
function SingleContent() {
  ...
  const getDynamicTitle = () => {
    ...
    if (parts.length > 0) return `Phim lẻ ${parts.join(" - ")}`;
    return "Phim Lẻ Mới Cập Nhật";
  };
...
export default function SingleRoute() {
  return (
    <MainLayout hideBreadcrumb={false}>
      <Navbar />
      <div>
        <Suspense fallback={<div className="p-10 text-text-muted text-center py-20 font-bold">Đang tải danh sách phim lẻ...</div>}>
          <SingleContent />
        </Suspense>
      </div>
    </MainLayout>
